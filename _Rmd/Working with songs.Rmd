...
kbl <- kable(song.feat[1:6, ], align = "c", row.names = F,  format = "html")
...
kbl <- kable(song.feat[1:6, -c(2:8)], align = "c", row.names = F,  format = "html")
...
# add song data
elm.sp <- merge(elm.sp[, -c(3:4)], pe_st, by = c("sound.files", "selec"))
...
xc <- xcorr(song.feat[1:10, ], parallel = 3)
