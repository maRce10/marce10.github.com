...
The `auto.next` argument can help speed up the process once users field confortable with the function. The `pause` argument (not shown) controls how long the function waits until it moves into the next selection
